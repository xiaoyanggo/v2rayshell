...
pre_install_docker_compose(){
    # Set ssrpanel_url
    echo "请输入网站域名："
    read -p "(没有默认值，请确保您输入正确的内容):" ssrpanel_url
    [ -z "${ssrpanel_url}" ]
    echo
    echo "---------------------------"
    ...
    # Set ssrpanel key
    echo "请输入网站密钥："
    read -p "(没有默认值，请确保您输入正确的内容):" ssrpanel_key
    [ -z "${ssrpanel_key}" ]
    echo
    echo "---------------------------"
    ...
    echo "请输入测速周期："
    read -p "(sspanel speedtest：每次默认（6）小时):" ssrpanel_speedtest
    ...
    echo "设置V2ray后端API监听端口："
    read -p "（V2ray API监听端口（默认2333））:" v2ray_api_port
    ...
    echo "请设置前端面板异常下线时，节点是否下线（1，下线；0，不下线）："
    read -p "(如不设置默认是1):" v2ray_downWithPanel
...
pre_install_caddy(){
    ...
    # 设置caddyv2域名
    echo "caddy v2ray 的域名"
    read -p "(没有默认值，请确保您输入正确的内容):" v2ray_domain
    [ -z "${v2ray_domain}" ]
    echo
    echo "---------------------------"
    ...
    echo "caddy v2ray tls 的邮箱"
    read -p "(自己的邮箱就可以):" v2ray_email
    [ -z "${v2ray_email}" ]
    echo
    echo "---------------------------"
...
install_dependencies(){
    if check_sys packageManager yum; then
        ...
        [ ! -f /etc/yum.repos.d/epel.repo ] && echo -e "[${red}Error${plain}] 安装EPEL储存库失败，请检查。" && exit 1
        [ ! "$(command -v yum-config-manager)" ] && yum install -y yum-utils > /dev/null 2>&1
        [ x"$(yum-config-manager epel | grep -w enabled | awk '{print $3}')" != x"True" ] && yum-config-manager --enable epel > /dev/null 2>&1
        echo -e "[${green}Info${plain}] 检查EPEL储存库是否完成..."
